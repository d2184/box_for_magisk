...
update_geox() {
  ...
  case "${bin_name}" in
    ...
    *)
      ...
      geoip_url="https://github.com/d2184/geoip/raw/release/$(if [[ "${bin_name}" = "clash" && "${geodata_mode}" == "false" ]]; then echo "Country.mmdb"; else echo "geoip.dat"; fi)"
      geosite_file="${box_dir}/clash/GeoSite.dat"
      geosite_url="https://github.com/d2184/geosite/raw/release/geosite.dat"
      
      if [ "${update_geox}" = "true" ] && { log Info "daily updates geox"; } && update_file "${geoip_file}" "${geoip_url}" && update_file "${geosite_file}" "${geosite_url}"; then
...
update_kernel() {
  ...
  case "${bin_name}" in
    "sing-box")
      api_url="https://api.github.com/repos/d2184/sing-box/releases"
      url_down="https://github.com/d2184/sing-box/releases"
      if [ "$use_ghproxy" == true ]; then
        url_down="https://mirror.ghproxy.com/${url_down}"
        api_url="https://mirror.ghproxy.com/${api_down}"
      fi
      latest_version=$(busybox wget --no-check-certificate -qO- "${api_url}" | grep "tag_name" | grep -o "v[0-9].*" | head -1 | cut -d'"' -f1)
      download_link="${url_down}/download/${latest_version}/sing-box-${latest_version}-${platform}-${arch}.tar.gz"
